...
I could not find a concrete missing 10% in the current implementation. The items below are possible future work, not evidence that a required schema is absent:

- Validation with Schema Markup Validator and Google's Rich Results Test.
- Monitoring Search Console for structured-data warnings or enhancement reports.
- Adding a more specialized type (for example, `Article`, `WebPage`, or `CollectionPage`) only where a real page and use case justify it.

The roadmap does contain unfinished validation tasks, but it does not identify a specific missing schema component that accounts for exactly 10%. I therefore cannot defend the 90% figure as a current measurement.
...
Because I couldn't find an identifiable missing feature, I believe the roadmap's "90% complete" estimate is likely a snapshot from 2026-06-19 that has since become stale. Based on the current codebase, the structured data implementation appears functionally complete for the site's existing content. The defensible follow-up is to validate and monitor it, then record any real validator or Search Console finding rather than assigning an unsupported percentage.
...
I did **not** log into Bing Webmaster Tools during this review, so I cannot confirm whether the property is verified or which method was used. The repository has no Bing verification meta tag or other code-visible evidence, but that does not rule out DNS verification or Bing's GSC-import path. The status is therefore **unverified, not "not configured."**

Although no Bing verification meta tag or HTML verification file exists in the codebase, that alone is not sufficient to determine its status because Bing also supports importing a site directly from Google Search Console.
...
=>
Before reading the roadmap's examples, I independently picked **Services** as the weakest page. Its title felt generic and its description did not express the service intent strongly enough. I would rewrite it around terms such as "geotechnical investigation," "soil testing," and "foundation engineering Nepal."

After recording that independent choice, I compared it with the roadmap:

| Page         | Current/Original Title                                                   | Recommended Title                                                                 | Assessment                                                                                                                                                                                                                       |
| ------------ | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Homepage** | `NS Engineering - Geotechnical Services`                                 | `NS Engineering - Geotechnical Services in Nepal`                                 | The roadmap supports adding a location-focused keyword such as **"in Nepal."**
| **Services** | `Professional Geotechnical Services - Testing, Investigation & Analysis` | `Geotechnical Testing & Drilling Services - NS Engineering Nepal`                 | **Strongly supports the independent finding.** The recommended title is more specific about the services and geographic target.                                                                                                  |
| **Projects** | `Projects - [project count]+ Infrastructure Projects Across Nepal`       | `[project count] Infrastructure Projects - Hydropower, Roads, NDT Services Nepal` | Identifies a **second clear optimization opportunity**.                                                        
...
=>
After rerunning Lighthouse on all five production pages, all five pages received a 100/100 SEO score. Therefore, Lighthouse did not identify a current SEO failure that I could confidently describe as a surprising finding.

The rerun confirmed that important technical SEO checks, such as page titles, meta descriptions, HTTP status codes, crawlable links, robots.txt, image alt text, and canonical URLs, are currently passing. These are important to search-engine crawlers even though many of them are not directly visible to users because crawlers use this information to understand, index, and correctly interpret a webpage.

Because all five pages currently score 100, I should not claim that Lighthouse caught a specific hidden problem. Therefore, the most accurate finding is that the current Lighthouse audit found no SEO issues on the five tested pages.
---
