...
export async function copyMilestoneImages(config: MilestonesConfig): Promise<void> {
  ...
  console.log(`✅ Verified ${foundCount} milestone images`);
  if (missingCount > 0) {
    console.log(`⚠️  Missing ${missingCount} images (will use fallback)`);
  }
}
